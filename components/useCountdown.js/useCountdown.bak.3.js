...
export const useCountdown = ({updateInterval = 1000, initialDuration = 0} = {}) => {
    ...
    const [intervalTimestamp, setIntervalTimestamp] = useState(0);
        ...
        if (isRunning && remaining >= 0) {
            ...
            const step = (timestamp) => {
                ...
                if (previousTimestamp) {
                    // if previous timestamp, increment elapsed time
                    elapsed.current += timestamp - previousTimestamp;
                } else if (!intervalTimestamp) {
                    setIntervalTimestamp(timestamp)
                }
                // else { 
                ...
                if (remaining <= 0) {
                    ...
                    // 
                    const intervalDiff = timestamp - (intervalTimestamp || timestamp);
                    console.log('intervalTimestamp:', intervalTimestamp, ', intervalDiff:', intervalDiff);
                    if (updateInterval < 1000 && intervalDiff >= updateInterval) {
                        console.log('\tset intervalTimestamp:', timestamp);
                        setIntervalTimestamp(timestamp);
                    }
    ...
    }, [isRunning, reset, duration, updateInterval. seconds, intervalTimestamp]);
